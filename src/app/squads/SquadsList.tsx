...
import { SPORT_LABELS, TAIWAN_CITIES, SKILL_LABELS, type SportType, type SquadCard } from '@/lib/types'
import { format } from 'date-fns'
import { zhTW } from 'date-fns/locale'
import { useRouter } from 'next/navigation'

function SquadCardComponent({ squad }: { squad: SquadCard }) {
...
export default function SquadsList({ initialSquads }: { initialSquads: SquadCard[] }) {
  ...
  function applyFilters() {
    ...
    router.push(`/squads${params.toString() ? '?' + params.toString() : ''}`)
  }

  const filtered = initialSquads.filter(s => {
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        {filtered.length === 0 ? (
          <div className="text-center py-16">
            ...
            <Link
              href="/squads/new"
              className="inline-flex items-center gap-1.5 bg-sky-500 hover:bg-sky-600 text-white px-4 py-2 rounded-lg text-sm font-medium mt-4 transition-colors"
            >
              發起揪團
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map(squad => (
              <SquadCardComponent key={squad.id} squad={squad} />
            ))}
          </div>
        )}
      </div>
