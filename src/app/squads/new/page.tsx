        ...
        <div className="flex items-center justify-between">
          {STEPS.map((s, i) => (
            <div key={i} className="flex items-center gap-2" onClick={() => i < step && setStep(i)}>
              ...
              <span className={`text-xs hidden sm:block ${i === step ? 'text-orange-600 font-medium' : 'text-gray-400'}`}>
                {s.label}
              </span>
              {i < STEPS.length - 1 && (
                <div className={`flex-1 h-0.5 w-8 sm:w-12 mx-1 rounded ${i < step ? 'bg-orange-400' : 'bg-gray-200'}`} />
              )}
            </div>
          ))}
        </div>
          ...
          <div className="mt-8 flex items-center justify-between gap-3">
            ...
            {step < 3 ? (
              ...
              <button
                ...
                disabled={loading}
                className="flex items-center gap-1.5 bg-orange-500 hover:bg-orange-600 disabled:bg-orange-300 text-white px-6 py-2.5 rounded-xl text-sm font-semibold transition-colors"
              >
                {loading ? '發布中...' : '確認發布'}
