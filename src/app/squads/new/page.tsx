...
export default function NewSquadPage() {
  ...
  return (
    <div className="min-h-screen">
      {/* Header */}
      <div className="border-b border-[#d2d2d7]/60 sticky top-16 z-30 bg-[#ffffff]/80 backdrop-blur-md">
        <div className="max-w-3xl mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-bold text-[#1d1d1f]">發起揪團</h1>
              <p className="text-xs text-[#6e6e73]">設定揪團內容，邀請運動夥伴</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-6">
            ...
            <div key={i} className="flex items-center gap-2 flex-1 last:flex-none">
              <div
                ...
                className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold transition-all cursor-pointer ${
                  i < step
                    ? 'bg-[#0071e3] text-[#1d1d1f]'
                    : i === step
                    ? 'bg-[#0071e3] text-[#1d1d1f] shadow-sm'
                    : 'bg-[#f5f5f7] border border-[#d2d2d7]/60 text-[#86868b]'
                }`}
              >
                {i < step ? <Check className="w-4 h-4" /> : s.icon}
              </div>
              <span className={`text-xs hidden sm:block whitespace-nowrap ${
                i === step ? 'text-[#0071e3] font-medium' : i < step ? 'text-[#6e6e73]' : 'text-[#86868b]'
              }`}>
                {s.label}
              </span>
              {i < STEPS.length - 1 && (
                <div className={`flex-1 h-0.5 mx-2 rounded ${i < step ? 'bg-[#0071e3]' : 'bg-[#0071e3]/10'}`} />
              )}
            </div>
          ))}
        </div>
        ...
        {/* Form Card */}
        <div className="bg-[#f5f5f7] border border-[#d2d2d7]/60 rounded-2xl p-6 md:p-8">
          {/* Step 0: Sport & Title */}
          {step === 0 && (
            <div className="space-y-6">
              <div>
                <label className="flex items-center gap-1.5 text-sm font-semibold text-[#1d1d1f] mb-2">
                ...
                <input
                  ...
                  className={`w-full bg-[#f5f5f7] border rounded-xl px-4 py-3 text-sm text-[#1d1d1f] placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all ${
                    errors.title ? 'border-rose-500/50' : 'border-[#d2d2d7]/60 hover:border-[#d2d2d7]'
                  }`}
                />
                    ...
                    return (
                      <button
                        ...
                        className={`flex flex-col items-center gap-1.5 p-3.5 rounded-xl border-2 transition-all ${
                          form.sport === value
                            ? 'border-cyan-400 bg-[#0071e3]/10 shadow-sm'
                            : 'border-[#d2d2d7]/60 hover:border-white/30 bg-white/[0.02]'
                        }`}
                      >
                        <span className="text-2xl">{emoji}</span>
                        <span className={`text-xs font-medium ${form.sport === value ? 'text-[#0071e3]' : 'text-[#6e6e73]'}`}>{text}</span>
                      </button>
                    )
                  })}
                </div>
              ...
              <div>
                <label className="block text-sm font-semibold text-[#1d1d1f] mb-3">技術程度</label>
            ...
            <div className="space-y-6">
              <div>
                ...
                <input
                  ...
                  className={`w-full bg-[#f5f5f7] border rounded-xl px-4 py-3 text-sm text-[#1d1d1f] focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all [color-scheme:dark] ${
                    errors.scheduled_at ? 'border-rose-500/50' : 'border-[#d2d2d7]/60 hover:border-[#d2d2d7]'
                  }`}
                />
              ...
              <div>
                <label className="block text-sm font-semibold text-[#1d1d1f] mb-2">持續時間</label>
                <div className="grid grid-cols-4 gap-2">
              ...
              <div>
                ...
                {TAIWAN_CITIES.length > 8 && (
                  <details className="mt-2">
                    <summary className="text-xs text-[#0071e3] cursor-pointer hover:text-[#0071e3]">顯示更多 ({TAIWAN_CITIES.length - 8})</summary>
              ...
              {districts.length > 0 && (
                <div>
                  <label className="block text-sm font-semibold text-[#1d1d1f] mb-2">區域 <span className="text-[#86868b] text-xs font-normal">(選填)</span></label>
                  <div className="flex flex-wrap gap-2">
                    {districts.map(d => (
                      <button
                        key={d}
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full bg-[#f5f5f7] border rounded-xl px-4 py-3 text-sm text-[#1d1d1f] placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all ${
                    errors.location_detail ? 'border-rose-500/50' : 'border-[#d2d2d7]/60 hover:border-[#d2d2d7]'
                  }`}
                />
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="例如：球拍自備"
                  className="w-full bg-[#f5f5f7] border border-[#d2d2d7]/60 rounded-xl px-4 py-3 text-sm text-[#1d1d1f] placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all"
                />
              </div>
          ...
          {step === 3 && (
            <div className="space-y-5">
              <div className="text-center pb-4 border-b border-[#d2d2d7]/60">
                <span className="text-5xl">{sportEmoji}</span>
                <h3 className="text-xl font-bold text-[#1d1d1f] mt-3">{form.title || '未命名揪團'}</h3>
              ...
              <div className="space-y-3 text-sm">
                <div className="flex items-start gap-3 p-3 bg-[#f5f5f7] rounded-xl">
                  ...
                  <div>
                    <p className="text-[#6e6e73] text-xs">時間</p>
                    <p className="text-[#1d1d1f]">{form.scheduled_at ? new Date(form.scheduled_at).toLocaleString('zh-TW') : '未設定'} · {form.duration_minutes} 分鐘</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3 p-3 bg-[#f5f5f7] rounded-xl">
                  ...
                  <div>
                    <p className="text-[#6e6e73] text-xs">地點</p>
                ...
                <div className="flex items-start gap-3 p-3 bg-[#f5f5f7] rounded-xl">
                  ...
                  <div>
                    <p className="text-[#6e6e73] text-xs">人數 / 費用</p>
                    <p className="text-[#1d1d1f]">
                      最多 {form.max_participants} 人 · {form.price_per_person === 0 ? <span className="text-[#00873e] font-bold">免費</span> : `$${form.price_per_person} / 人`}
                    </p>
                  </div>
                </div>

                {form.notes && (
                  <div className="p-3 bg-[#ff9500]/10 border border-amber-500/20 rounded-xl">
                    <p className="text-[#ff9500] text-xs mb-1">⚠️ 注意事項</p>
                    <p className="text-[#1d1d1f] text-sm">{form.notes}</p>
                  </div>
                )}
              </div>
              ...
              {errors.submit && (
                <div className="p-3 bg-rose-500/10 border border-rose-500/30 rounded-xl">
                  <p className="text-[#ff3b30] text-sm">❌ {errors.submit}</p>
                </div>
              )}
            </div>
          )}
        ...
        <div className="flex items-center justify-between mt-6">
          <button
            ...
            className="px-5 py-2.5 rounded-xl text-sm font-medium text-[#1d1d1f] hover:text-[#1d1d1f] border border-[#d2d2d7]/60 hover:border-[#d2d2d7] transition-all"
          >
            {step === 0 ? '取消' : '上一步'}
          </button>
          ...
          {step < STEPS.length - 1 ? (
            <button
              ...
              className="inline-flex items-center gap-1.5 bg-[#0071e3] hover:from-cyan-400 hover:to-sky-400 text-[#1d1d1f] px-6 py-2.5 rounded-xl text-sm font-bold transition-all shadow-sm"
            >
              下一步
            ...
            <button
              ...
              disabled={loading}
              className="inline-flex items-center gap-1.5 bg-[#0071e3] hover:from-cyan-400 hover:to-sky-400 disabled:opacity-50 text-[#1d1d1f] px-7 py-2.5 rounded-xl text-sm font-bold transition-all shadow-sm"
            >
              {loading ? '發布中...' : '確認發布'}
