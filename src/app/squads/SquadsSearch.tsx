              ...
              <div key={step.num} className="flex items-center gap-0 flex-1">
                ...
                <span
                  className={`text-xs ml-1 hidden sm:block ${
                    currentStep >= step.num ? 'text-orange-600 font-medium' : 'text-gray-400'
                  }`}
                >
                  {step.label}
                </span>
                {i < steps.length - 1 && (
                  <div
                    className={`flex-1 h-0.5 mx-2 rounded ${
                      currentStep > step.num ? 'bg-orange-400' : 'bg-gray-200'
                    }`}
                  />
                )}
              </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
              {availableCities.map(city => (
                ...
                >
                  <MapPin className={`w-6 h-6 ${selectedCity === city ? 'text-orange-500' : 'text-gray-400'}`} />
                  <span className={`text-sm font-medium ${selectedCity === city ? 'text-orange-600' : 'text-gray-700'}`}>
                    {city}
                  </span>
                </button>
              ))}
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
              {availableDistricts.map(district => (
                ...
                >
                  ...
                  <span className={`text-xs font-medium ${selectedDistrict === district ? 'text-orange-600' : 'text-gray-700'}`}>
                    {district}
                  </span>
                </button>
              ))}
              ...
              {sports.map(([value, label]) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`font-semibold ${selectedSport === value ? 'text-orange-600' : 'text-gray-800'}`}>
                      {text}
                    </span>
                  </button>
                )
        ...
        {currentStep === 4 && (
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2 text-sm">
              <span className="bg-orange-100 text-orange-700 px-3 py-1 rounded-full font-medium">{selectedCity}</span>
              <ArrowRight className="w-4 h-4 text-gray-400" />
              <span className="bg-orange-100 text-orange-700 px-3 py-1 rounded-full font-medium">{selectedDistrict}</span>
              <ArrowRight className="w-4 h-4 text-gray-400" />
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="text-xs text-gray-500 hover:text-orange-500 underline"
              >
                重新選擇
              </button>
            </div>
            ...
            {squads.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
                ...
                <Link
                  href="/squads/new"
                  className="inline-flex items-center gap-1.5 bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors"
                >
                  成為第一個發起的人
                </Link>
              </div>
                ...
                {squads.map(squad => {
                  ...
                  return (
                    <Link
                      ...
                      className="block bg-white rounded-2xl border border-gray-100 hover:border-orange-200 hover:shadow-md transition-all overflow-hidden"
                    >
                      <div className="flex">
                        {/* Left color bar */}
