...
function SquadCardHome({ squad }: { squad: ReturnType<typeof getAllSquadCards>[0] }) {
  ...
  return (
    <Link
      ...
      className="group block bg-white rounded-3xl border-2 border-white/10 hover:border-cyan-500/40 hover:shadow-xl transition-all duration-200 overflow-hidden"
    >
      ...
      {/* Content */}
      <div className="p-5">
        <h3 className="font-bold text-base mb-3 group-hover:text-cyan-300 transition-colors" className="text-white">
          {squad.title}
        </h3>

        <div className="space-y-1.5 text-sm" className="text-white/60">
        ...
        <div className="mt-4 pt-3 flex items-center justify-between" className="border-t border-dashed border-white/10">
          <span className={`text-sm font-bold ${isFree ? 'text-green-600' : 'text-cyan-300'}`}>
            {isFree ? '✨ 免費' : `${squad.price_per_person}元/人`}
          </span>
          <span className="text-xs" className="text-white/60">
...
function StatCard({ icon: Icon, value, label }: { icon: any; value: string; label: string }) {
  return (
    <div className="bg-white rounded-3xl border border-white/10 p-6 text-center hover:shadow-lg transition-shadow">
      ...
      <div className="text-3xl font-black mb-1" className="text-white">{value}</div>
      <div className="text-sm" className="text-white/60">{label}</div>
    </div>
  )
}
  ...
  return (
    <div className="min-h-screen">
      {/* ============================================================
          1. HERO
          ============================================================ */}
      <section className="relative overflow-hidden bg-gradient-to-br from-cyan-500/20 via-sky-600/10 to-transparent">
        ...
        <div className="relative max-w-6xl mx-auto px-4 py-20 md:py-28">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 bg-cyan-500/10 border border-cyan-500/30 rounded-full px-4 py-2 mb-6 shadow-sm">
              <Sparkles className="w-4 h-4 text-cyan-400" />
              <span className="text-sm font-bold" className="text-cyan-300">
                847 位運動夥伴都在用
              </span>
            </div>
            <h1 className="text-4xl md:text-6xl font-black leading-tight mb-6 " className="text-white">
              找到對的運動夥伴<br />
              <span className="text-cyan-400">一起流的汗最好玩</span>
            </h1>
            <p className="text-lg md:text-xl mb-8 leading-relaxed" className="text-white/70">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <Link
                href="/squads"
                className="inline-flex items-center justify-center gap-2 bg-white hover:bg-white/5 text-cyan-300 px-8 py-4 rounded-2xl font-bold text-lg border-2 border-white/20 transition-all"
              >
                瀏覽附近揪團
              </Link>
            </div>
      ...
      <section className="max-w-6xl mx-auto px-4 mb-20">
        <div className="text-center mb-10">
          <h2 className="text-3xl md:text-4xl font-black mb-3" className="text-white">
            你是哪一種運動咖？<span className="text-cyan-400">👇</span>
          </h2>
          <p className="text-base md:text-lg" className="text-white/60">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {SCENARIO_CARDS.map(card => (
            ...
            >
              ...
              <h3 className="font-bold text-base mb-2" className="text-white">
                {card.title}
              </h3>
              <p className="text-sm mb-4 leading-relaxed" className="text-white/70">
                {card.desc}
              </p>
      ...
      <section className="max-w-6xl mx-auto px-4 mb-16">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-2xl md:text-3xl font-black" className="text-white">
            🏆 探索運動
          </h2>
          ...
          {SPORT_LIST.map(sport => {
            ...
            return (
              <Link
                ...
                className="flex flex-col items-center gap-2 bg-white border-2 border-white/10 hover:border-cyan-500/40 hover:shadow-lg rounded-3xl px-6 py-4 min-w-[88px] transition-all"
              >
                <span className="text-3xl">{emoji}</span>
                <span className="font-bold text-sm" className="text-white">{text}</span>
              </Link>
            )
          })}
        </div>
      ...
      {/* ============================================================
          ...
          ============================================================ */}
      <section className="bg-white/[0.02] border-y border-white/10 py-20 mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 relative">
            {HOW_STEPS.map((step, idx) => (
              <div key={step.num} className="relative">
                <div className="bg-white rounded-3xl p-6 border border-white/10 hover:shadow-xl transition-shadow h-full">
                  ...
                  <h3 className="font-bold text-base mb-2" className="text-white">{step.title}</h3>
                  <p className="text-sm leading-relaxed" className="text-white/60">{step.desc}</p>
                </div>
      ...
      <section className="max-w-6xl mx-auto px-4 mb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {TRUST_PILLARS.map(pillar => (
            <div
              key={pillar.title}
              className="bg-white rounded-3xl p-7 border-2 border-white/10 hover:border-cyan-500/40 hover:shadow-xl transition-all"
            >
              ...
              <h3 className="font-bold text-lg mb-2" className="text-white">{pillar.title}</h3>
              <p className="text-sm leading-relaxed" className="text-white/60">{pillar.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 mb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {mockTestimonials.map(t => (
            <div
              key={t.id}
              className="bg-white rounded-3xl p-6 border border-white/10 hover:shadow-xl transition-shadow relative"
            >
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <div className="font-bold text-sm" className="text-white">{t.name}</div>
                  <div className="text-xs" className="text-white/60">
              ...
              <div className="flex gap-0.5 mb-3">
                {Array.from({ length: t.rating }).map((_, i) => (
                  <Star key={i} className="w-4 h-4 text-amber-400 fill-yellow-500" />
                ))}
              </div>
              <p className="text-sm leading-relaxed" className="text-white/60">
                &ldquo;{t.quote}&rdquo;
              </p>
            </div>
          ))}
      ...
      <section className="max-w-3xl mx-auto px-4 mb-20">
        ...
        <div className="space-y-3">
          {FAQS.map((faq, idx) => (
            <details
              key={idx}
              className="group bg-white rounded-2xl border-2 border-white/10 hover:border-cyan-500/40 transition-all"
            >
              <summary className="flex items-center justify-between gap-3 p-5 cursor-pointer font-bold list-none" className="text-white">
              ...
              <div className="px-5 pb-5 text-sm leading-relaxed" className="text-white/60">
                {faq.a}
              </div>
            </details>
          ))}
        ...
        <div className="max-w-5xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div className="bg-white rounded-3xl p-6 shadow-sm">
              ...
              <h3 className="font-bold text-lg mb-2" className="text-white">73% 的人想動但沒伴</h3>
              <p className="text-sm leading-relaxed" className="text-white/60">
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm">
              <div className="text-4xl mb-3">🤝</div>
              <h3 className="font-bold text-lg mb-2" className="text-white">5,000+ 個新友誼</h3>
              <p className="text-sm leading-relaxed" className="text-white/60">
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm">
              <div className="text-4xl mb-3">🌸</div>
              <h3 className="font-bold text-lg mb-2" className="text-white">女性友善揪團</h3>
              <p className="text-sm leading-relaxed" className="text-white/60">
      ...
      <section className="max-w-5xl mx-auto px-4 mb-12">
        <div className="bg-gradient-to-br from-cyan-500/20 via-sky-600/15 to-teal-500/15 border border-cyan-500/30 rounded-3xl p-10 md:p-14 text-center shadow-2xl">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link
              href="/squads/new"
              className="inline-flex items-center justify-center gap-2 bg-white text-cyan-300 hover:bg-white/5 px-8 py-4 rounded-2xl font-bold text-lg shadow-lg transition-all"
            >
              🔥 免費發起揪團
              <ArrowRight className="w-5 h-5" />
            </Link>
